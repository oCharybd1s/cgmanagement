...
export function DeleteLaporanDialog({
  report,
  onClose,
  onDeleted,
}: {
  ...
  async function handleDelete() {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/meeting-reports/${report.id}`, { method: "DELETE" });
      const data = await response.json();

      if (!response.ok || !data.ok) {
        setError(data.error ?? "Gagal menghapus Laporan CG");
        setIsDeleting(false);
        return;
      }
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          role="alertdialog"
          aria-modal="true"
          aria-labelledby="delete-laporan-dialog-title"
        ...
        >
          <div className="flex items-center justify-between border-b border-border px-6 py-4">
            <h2
              id="delete-laporan-dialog-title"
          ...
          <div className="flex flex-col gap-4 px-6 py-5">
            <div className="flex items-start gap-3 rounded-2xl border border-destructive/30 bg-destructive/10 p-4">
              ...
              <p className="text-sm text-foreground">
                Laporan pertemuan tanggal <span className="font-semibold">{report.meetingDate ?? "-"}</span> akan
