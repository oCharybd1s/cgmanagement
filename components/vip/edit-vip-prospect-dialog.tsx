...
export function EditVipProspectDialog({
  prospect,
  cgGroups,
  members,
  cgLabel,
  ...
  onClose: () => void;
  onUpdated: (prospect: VipProspect) => void;
}) {
  const [isSubmitting, setIsSubmitting] = React.useState(false);
  const [formError, setFormError] = React.useState<string | null>(null);
  const [fieldErrors, setFieldErrors] = React.useState<VipProspectFieldErrors>({});
  const [selectedCgId, setSelectedCgId] = React.useState(prospect.cgId ?? "");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        setFormError(data.error ?? "Gagal menyimpan perubahan data VIP");
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          role="dialog"
          aria-modal="true"
          aria-labelledby="edit-vip-dialog-title"
        ...
        >
          <div className="flex items-center justify-between border-b border-border px-6 py-4">
            <h2 id="edit-vip-dialog-title" className="font-display text-lg font-bold tracking-tight text-foreground">
              Ubah Data VIP
            </h2>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit} className="flex flex-1 flex-col gap-4 overflow-y-auto px-6 py-5">
            ...
            <Field label="Follow-up Oleh" htmlFor="followUpByUserId">
              <select
                id="followUpByUserId"
                name="followUpByUserId"
                defaultValue={prospect.followUpByUserId ?? ""}
                disabled={isSubmitting}
                className={inputClass}
              >
