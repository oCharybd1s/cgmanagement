...
export function DeleteVipProspectDialog({
  prospect,
  onClose,
  onDeleted,
}: {
  ...
  async function handleDelete() {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/vip-prospects/${prospect.id}`, { method: "DELETE" });
      const data = await response.json();

      if (!response.ok || !data.ok) {
        setError(data.error ?? "Gagal menghapus data VIP");
        setIsDeleting(false);
        return;
      }
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          role="alertdialog"
          aria-modal="true"
          aria-labelledby="delete-vip-dialog-title"
        ...
        >
          <div className="flex items-center justify-between border-b border-border px-6 py-4">
            <h2 id="delete-vip-dialog-title" className="font-display text-lg font-bold tracking-tight text-foreground">
              Hapus Data VIP
            </h2>
            <button
              type="button"
          ...
          <div className="flex flex-col gap-4 px-6 py-5">
            <div className="flex items-start gap-3 rounded-2xl border border-destructive/30 bg-destructive/10 p-4">
              ...
              <p className="text-sm text-foreground">
                Data <span className="font-semibold">{prospect.name || "prospek ini"}</span> akan dihapus permanen
                dari pipeline VIP. Tindakan ini tidak bisa dibatalkan.
              </p>
            </div>
