...
export function DeleteEventDialog({
  ...
  React.useEffect(() => {
    function handleKeyDown(keyEvent: KeyboardEvent) {
      if (keyEvent.key === "Escape" && !isDeleting) {
        onClose();
      }
    }
          ...
          <div className="flex flex-col gap-4 px-6 py-5">
            <div className="flex items-start gap-3 rounded-2xl border border-destructive/30 bg-destructive/10 p-4">
              ...
              <p className="text-sm text-foreground">
                Event <span className="font-semibold">{event.name || "ini"}</span> akan dihapus permanen dari
                kalender. Tindakan ini tidak bisa dibatalkan.
              </p>
            </div>
