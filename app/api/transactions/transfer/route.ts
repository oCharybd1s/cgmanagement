...
import { createTransferForSession } from "@/lib/transactions/transfer";
...
export async function POST(request: NextRequest) {
  ...
  const result = await createTransferForSession(session, body);
  ...
  return NextResponse.json({ ok: true, transactions: result.transactions });
}
